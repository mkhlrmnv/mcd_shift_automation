...
CLIENT_file = None #Your google OAuth2.0 .json file

creds = None

if os.path.exists('token.json'):
    creds = Credentials.from_authorized_user_file('token.json', SCOPES)  #checks if user has already allowed program to use google api
...
def main():
    ...
    scraper = defs.ShiftScraper()
    scraper.get_credentials()  #logins into mcd shift website
    scraper.login()
    
    shifts = scraper.get_shifts()  #gets shifts and dates
    dates = scraper.get_dates()
    
    scraper.quit()  #closes shift website
    ...
    for i in shifts:  #for every date in three week period checks if there is a shift
        if i != -1:  #if there is creates event and sends it to google calendar
            ...
            else:   #creates event if it is not a night shift
                event = {
                    'summary': 'Work',
                    'start': {'dateTime': f'{date}T{start_hour}:{start_minute}:00+03:00',
                        'timeZone': 'Europe/Helsinki',
